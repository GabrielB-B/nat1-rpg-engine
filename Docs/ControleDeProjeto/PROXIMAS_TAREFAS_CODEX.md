...
| 12 / G1 | `front/cartographer-brand-tokens` | em andamento | Consolidar marca Cartógrafo, tipografia, tokens semânticos, contraste e QA visual. | A fundação visual está implementada como candidata/provisória; preservar geometria entre temas e não tratar os ativos como marca final ou liberada comercialmente. O plugin Figma e o MCP remoto estão habilitados; o OAuth foi concluído anteriormente no host, mas a extensão requer reinício e nova thread para expor as ferramentas. | Especificação e implementação alinhadas; inventário/licenças registrados; pares operacionais AA, foco e reduced motion verificados; QA visual com evidências nos breakpoints; aprovação explícita de Gabriel. |
...
- G1 visual está em andamento em `front/cartographer-brand-tokens`; QA técnico e autenticação responsiva/temática passaram, mas faltam prancha óptica, lockup vertical, sidebar/empty state, simulações perceptivas e aprovação explícita de Gabriel.
- A fundação visual desta branch é candidata/provisória. Ela não representa logo final nem liberação comercial.
- O plugin Figma e o MCP remoto estão habilitados no Codex; o OAuth foi concluído anteriormente no host e a extensão precisa ser reiniciada, com retomada em nova thread, para que as ferramentas sejam carregadas.
- Home real e shell continuam em branches próprias e só iniciam após o encerramento formal de G1.
